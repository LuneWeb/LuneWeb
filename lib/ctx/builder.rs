...
impl ContextBuilder {
    ...
    pub fn lune_ctx(mut self, ctx_builder: GlobalsContextBuilder) -> Self {
        self.lune_ctx_builder = Some(ctx_builder);
        self
    }

    pub fn luau_input<T: Into<PathBuf>>(mut self, input: T) -> Self {
    ...
    pub fn js_inputs(mut self, inputs: Vec<PathBuf>) -> Self {
        for path in &inputs {
        ...
        self.js_inputs.append(&mut inputs.clone());
        self
    }
}
